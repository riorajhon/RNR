...
$stmt = $pdo->prepare("SELECT bs.redirect_url_positive, u.name AS business_name FROM business_settings bs JOIN users u ON u.id = bs.user_id WHERE bs.user_id = ?");
$stmt->execute([$user_id]);
$row = $stmt->fetch();
$business_name = $row ? ($row['business_name'] ?? 'Us') : 'Us';
$review_url = $row && !empty(trim($row['redirect_url_positive'] ?? '')) ? trim($row['redirect_url_positive']) : null;
...
<div class="reviews-page">
  ...
  $facebook_url = $url ?: 'https://www.facebook.com/';
  $yelp_url     = $url ?: 'https://www.yelp.com/writeareview';
  $tripadvisor_url = $url ?: 'https://www.tripadvisor.com/';
  ?>
  <div class="reviews-platforms">
    <a href="<?php echo htmlspecialchars($google_url); ?>" target="_blank" rel="noopener noreferrer" class="reviews-google" title="Leave a review on Google" aria-label="Google"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#fff"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#fff"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#fff"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#fff"/></svg></a>
    <a href="<?php echo htmlspecialchars($facebook_url); ?>" target="_blank" rel="noopener noreferrer" class="reviews-facebook" title="Leave a review on Facebook" aria-label="Facebook"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg></a>
    <a href="<?php echo htmlspecialchars($yelp_url); ?>" target="_blank" rel="noopener noreferrer" class="reviews-yelp" title="Leave a review on Yelp" aria-label="Yelp"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M20.16 12.73l-4.88 2.12 1.44 5.92c.18.72-.49 1.21-1.1.82l-4.2-2.78-4.2 2.78c-.61.4-1.28-.1-1.1-.82l1.44-5.92-4.88-2.12c-.62-.27-.45-1.1.24-1.2l5.92-.48 2.28-5.6c.28-.68 1.23-.68 1.51 0l2.28 5.6 5.92.48c.69.1.86.93.24 1.2z"/></svg></a>
    <a href="<?php echo htmlspecialchars($tripadvisor_url); ?>" target="_blank" rel="noopener noreferrer" class="reviews-tripadvisor" title="Leave a review on TripAdvisor" aria-label="TripAdvisor"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12.006 4.295c-2.67 0-5.338.784-7.645 2.353H0v12.706h4.35v-8.28c1.22-1.15 2.65-1.74 4.31-1.74 1.66 0 3.09.59 4.31 1.74v8.28h4.35V9.648c0-2.88-2.34-5.353-5.355-5.353zm12.006 0c-2.67 0-5.338.784-7.645 2.353H12v2.353h.01c1.12 0 2.23.25 3.24.73 1.01.48 1.92 1.17 2.7 2.04.78.87 1.4 1.9 1.84 3.06.44 1.16.66 2.4.66 3.7 0 2.88 2.34 5.35 5.35 5.35 2.67 0 5.34-.78 7.65-2.35V4.295h-4.35v8.28c-1.22 1.15-2.65 1.74-4.31 1.74-1.66 0-3.09-.59-4.31-1.74V4.295h-4.35z"/></svg></a>
  </div>
  <p class="reviews-platforms reviews-label"><?php echo $review_url ? 'Choose a platform above to leave your review' : 'Click a platform to leave your review'; ?></p>
